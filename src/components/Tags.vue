<template>
    <div class="d-flex container-fluid h-100 v-100 p-0">
        <div class="d-flex flex-column justify-content-center gap-3 w-50 h-100 border-end py-4"
            style="padding-left:5%; padding-right: 5%">
            <div class="d-flex flex-column align-items-center gap-3 w-100 h-auto">
                ...
                <div class="form-group w-100">
                    ...
                    <input v-model="selectedPriorityName" type="text" class="form-control"
                        placeholder="Asunto del ticket">
                </div>

                <button type="button" class="btn btn-success w-100">{{ selectedPriorityID == -1 ? 'Crear Prioridad' :
                    'Guardar prioridad' }}</button>
                <button type="button" class="btn btn-danger w-50"
                    :style="{ visibility: selectedPriorityID == -1 ? 'hidden' : 'visible' }">Eliminar Prioridad</button>
            </div>
        </div>
        <div class="d-flex flex-column justify-content-center gap-3 w-50 h-100 border-start py-4"
            style="padding-left:5%; padding-right: 5%">
            <div class="d-flex flex-column align-items-center gap-3 w-100 h-auto">
                ...
                <div class="d-flex flex-column w-100">
                    ...
                    <select v-model="selectedStatusID" class="text-reset btn btn-outline-secondary text-start"
                        ...
                        <option v-for="stat in statusList" style="background-color: #212529;"
                            :value="stat.ID_status" @click="setStatusName(stat.tipostatus)">
                ...
                <div class="form-group w-100">
                    ...
                    <input v-model="selectedStatusName" type="text" class="form-control"
                        placeholder="Asunto del ticket">
                </div>

                <button type="button" class="btn btn-success w-100" @click.prevent="postStatus()">{{ selectedStatusID == -1 ? 'Crear estatus' :
                    'Guardar estatus' }}</button>
                <button type="button" class="btn btn-danger w-50"
                    :style="{ visibility: selectedStatusID == -1 ? 'hidden' : 'visible' }" @click="deleteComment()">Eliminar estatus</button>
            </div>
        </div>
    </div>
</template>
